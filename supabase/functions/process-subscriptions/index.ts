...
Deno.serve(async () => {
  ...
  const { data: subscriptions } = await supabase.from('wc_subscriptions').select('id,subscriber_id,creator_id,tier_id').eq('status', 'active').lte('renews_at', now)
  ...
  for (const subscription of subscriptions ?? []) {
    ...
    if (charge.ok) { const renewsAt = new Date(); renewsAt.setMonth(renewsAt.getMonth() + 1); await supabase.from('wc_subscriptions').update({ status: 'active', renews_at: renewsAt.toISOString() }).eq('id', subscription.id) }
    else await supabase.from('wc_subscriptions').update({ status: 'past_due' }).eq('id', subscription.id)
  }
